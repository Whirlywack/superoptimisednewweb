...
describe('FeatureVoting', () => {
  ...
  it('renders description when provided', () => {
    render(
      <FeatureVoting 
        {...defaultProps} 
        description="Allocate points to show preference" 
      />
    );
  ...
  it('prevents allocation beyond total points', async () => {
    ...
    render(<FeatureVoting {...defaultProps} onChange={mockOnChange} value={initialValue} totalPoints={100} />);
    
    const input = screen.getByLabelText('Points for Feature 2');
    await user.clear(input);
    await user.type(input, '20'); // Would exceed 100 total
    
    // Should only allocate remaining 10 points
    expect(mockOnChange).toHaveBeenLastCalledWith([
      { optionId: 'feature1', points: 90 },
      { optionId: 'feature2', points: 10 }
    ]);
  });
  ...
  it('shows validation error when provided', () => {
    render(
      <FeatureVoting 
        {...defaultProps} 
        error="Please allocate at least some points" 
      />
    );
  ...
  it('shows helper text when provided', () => {
    ...
    expect(screen.getByText('Allocate more points to higher priority features')).toBeInTheDocument();
  });
