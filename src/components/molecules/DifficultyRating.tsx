...
import { Circle, Square, Triangle, Hexagon, Star, TrendingUp, Brain } from "lucide-react";
...
export function DifficultyRating({
  ...
  const renderOption = (option: DifficultyOption, _index: number) => {
    const isSelected = selectedLevel === option.level;
    const isBeforeSelected = selectedLevel && option.level <= selectedLevel;

    return (
      <div key={option.level} className={cn(orientation === "vertical" ? "w-full" : "flex-1")}>
        <button
          type="button"
          className={cn(
          ...
          <div className="flex items-center space-x-3">
            ...
            <div
              className={cn(
                ...
                variant === "stars" && isBeforeSelected
                  ? option.color
                  : isSelected
                    ? option.color
                    : "text-warm-gray"
            ...
            <div className="min-w-0 flex-1">
              <div className="font-medium text-off-black dark:text-off-white">{option.label}</div>
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      {variant === "stars" && !showDescriptions ? (
        ...
        <div
          className={cn("space-y-3", orientation === "horizontal" && "flex space-x-3 space-y-0")}
        >
      ...
      <div className="flex items-start space-x-2 rounded-lg bg-light-gray p-3 dark:bg-warm-gray/20">
        ...
        <div className="text-sm text-off-black dark:text-off-white">
          <strong>Rating Guide:</strong> Consider technical complexity, required expertise, time
          investment, and potential risks when rating difficulty.
        </div>
      </div>
