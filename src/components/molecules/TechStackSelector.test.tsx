...
jest.mock('@/components/ui/Input', () => ({
  Checkbox: ({ checked, onChange, disabled, children }: { checked: boolean; onChange: () => void; disabled?: boolean; children?: React.ReactNode }) => (
    <input
...
jest.mock('@/components/ui/Tag', () => ({
  Tag: ({ children, onRemove, removable }: { children: React.ReactNode; onRemove?: () => void; removable?: boolean }) => (
    <span data-testid="tag">
      {children}
      {removable && onRemove && (
        <button data-testid="remove-tag" onClick={onRemove}>×</button>
      )}
    </span>
  ),
}));
...
describe('TechStackSelector', () => {
  ...
  it('handles deselection', async () => {
    ...
    render(<TechStackSelector {...defaultProps} onChange={mockOnChange} value={['react', 'vue']} />);
  ...
  it('preserves selection state correctly', () => {
    const { rerender } = render(
      <TechStackSelector {...defaultProps} value={['react']} />
    );
    ...
    // Update selection
    rerender(
      <TechStackSelector {...defaultProps} value={['react', 'vue']} />
    );
