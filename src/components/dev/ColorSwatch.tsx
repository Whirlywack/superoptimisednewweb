...
export function ColorSwatch({ color, size = "md", showCopy = true, className }: ColorSwatchProps) {
  ...
  return (
    <div className={cn("flex flex-col space-y-2", className)}>
      ...
      <div className="relative">
        <div
          className={cn("rounded-full border-2 border-light-gray shadow-sm", sizes[size])}
      ...
      <div className="space-y-1">
        <h4 className="text-small font-semibold text-off-black">{color.name}</h4>

        {color.description && <p className="text-xs text-warm-gray">{color.description}</p>}
        ...
        {showCopy && (
          <div className="space-y-1">
            ...
            <button
              ...
              className="flex w-full items-center justify-between rounded bg-light-gray p-2 text-xs transition-colors hover:bg-warm-gray/10"
            >
              <code className="font-mono">{color.value}</code>
              {copied === "hex" ? (
                <Check className="size-3 text-green-600" />
              ) : (
                <Copy className="size-3 text-warm-gray" />
              )}
            </button>
...
export function ColorPalette({
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      >
        {colors.map((color) => (
          <ColorSwatch key={color.name} color={color} size={size} />
        ))}
      </div>
    </div>
  );
...
export function ColorScale({
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className={cn("flex", direction === "horizontal" ? "flex-row" : "flex-col")}>
          ...
          <div key={color.name} className="group relative">
            <button
              className={cn(
                ...
                direction === "horizontal"
                  ? "size-16 first:rounded-l-lg last:rounded-r-lg"
            ...
            {showLabels && (
              <div
                className={cn(
                  "absolute z-10 rounded bg-off-black p-2 text-xs text-off-white shadow-lg",
      ...
      {selectedColor && (
        <div className="rounded-lg bg-light-gray p-4">
          <h4 className="mb-2 font-semibold text-off-black">Selected: {selectedColor.name}</h4>
...
export function AccessibilityCheck({ foreground, background, className }: AccessibilityCheckProps) {
  // Simple contrast ratio calculation (for demo purposes)
  const calculateContrast = (_fg: string, _bg: string): number => {
  ...
  return (
    <div className={cn("rounded-lg border border-light-gray p-4", className)}>
      <h4 className="mb-3 font-semibold text-off-black">Accessibility Check</h4>

      {/* Color Preview */}
      <div
        className="mb-3 rounded p-4 text-center"
        style={{
