...
import { Send, Mic, MicOff, X, Smile, Eye, EyeOff } from "lucide-react";

export type OneHandedInputType =
  | "text"
  | "textarea"
  | "email"
  | "password"
...
export function OneHandedInput({
  ...
  const canSubmit =
    currentValue.trim().length > 0 && (!minLength || currentValue.length >= minLength);
  ...
  return (
    <div className={cn(getPositionStyles(), className)}>
      ...
      >
        ...
        {validationMessage && (
          <div
            className={cn("mb-3 rounded-lg px-4 py-2 text-sm", {
              ...
              <div className="flex items-center gap-1 pl-2">
                {allActions.map((action) => (
                  <button
                    ...
                    className={cn(
                      ...
                      sizeStyles.button,
                      {
                        "bg-primary text-white":
                          action.active && (!action.variant || action.variant === "default"),
                        "text-warm-gray hover:bg-primary/10 hover:text-primary":
                          !action.active && !action.disabled,
                        "cursor-not-allowed opacity-50": action.disabled || disabled,
                    ...
                    title={action.label}
                  >
                    <Icon size={sizeStyles.iconSize >= 22 ? "lg" : "md"}>{action.icon}</Icon>
                  </button>
                ))}
              </div>
            )}
                  ...
                  className={cn(
                    ...
                    sizeStyles.button,
                    {
                      "bg-primary text-white hover:bg-primary/90 active:scale-95":
                        canSubmit && !disabled,
                      "cursor-not-allowed bg-light-gray text-warm-gray": !canSubmit || disabled,
                    }
                  )}
            ...
            {showCounter && (
              <span
                className={cn({
                  ...
                  "text-yellow-500":
                    maxLength &&
                    currentValue.length > maxLength * 0.75 &&
