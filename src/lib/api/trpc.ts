...
const adminMiddleware = t.middleware(async ({ ctx, next }) => {
  ...
  if (ctx.session.user.role !== "admin" && !ctx.session.user.isAdmin) {
    throw new TRPCError({
      code: "FORBIDDEN",
...
/**
 * Admin procedure - only accessible to authenticated admin users
 *
 ...
 * @see https://trpc.io/docs/procedures
 */
export const adminProcedure = t.procedure.use(loggingMiddleware).use(adminMiddleware);
